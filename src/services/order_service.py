...
async def calculate_total_price(
    ...
    for order_item in order_items:
        ...
        if product:
            ...
            discount_percentage = await get_discount_percentage(
                product.category, customer, db
            )
...
async def get_discount_percentage(
    ...
    result = await db.execute(
        select(Discount).filter_by(
            customer_category=customer.category, product_category_id=product_category.id
        )
    )
